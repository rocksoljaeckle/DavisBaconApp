...
## Task Overview
You will be provided a file containing a Davis-Bacon wage determination, and a file containing the payroll for a construction project. If you cannot parse the provided payroll file, you will need to respond with JSON indicating the failure. Otherwise, you will respond with JSON containing the results of your analysis. See below for details on the exact output format.
...
- Whether the payroll contains a statement of compliance that certifies the required items
...
If you *are* able to parse the payroll file, provide your analysis as a JSON object with the following top-level attributes:
- 'success': true
...
- 'has_compliance_statement': Whether the payroll has a statement of compliance that certifies the required items (boolean) - see below for details
...
Set to true only if all calculations verify correctly. Set to false if any calculation errors are found. If deductions cannot be verified due to missing itemization, note this in compliance_reasoning but do not mark as non-compliant solely for this reason


## Statement of Compliance Certification

The 'has_compliance_statement' field indicates whether the payroll contains a statement of compliance that certifies all of the following:
...
```json
{
  ...
  "properties": {
    ...
    "has_compliance_statement": {
      "title": "Has Compliance Statement",
      "type": "boolean"
    },
    "notes": {
      "title": "Notes",
...
Here is a sample output following the specified format:
```json
{
  "success": true,
...
- If there is no work listed on the payroll, respond with success: true and an empty wage_checks array, but still populate the payroll-level fields (is_one_week, has_contract_number, mathematically_correct, signed, has_compliance_statement)
